...
void UCTSearch::dump_GUI_stats(GameState & state, UCTNode & parent) {
    ...
    while (node != nullptr) {
        if (node->get_score() > 0.005f || node->get_visits() > 0) {
            ...
            TRowVector row;
#ifdef JP
            row.emplace_back(std::string("着手"), movestr);
            row.emplace_back(std::string("探索比率%"),
                std::to_string(100.0 * node->get_visits() / (double)total_visits));
            row.emplace_back(std::string("試行回数"),
                std::to_string(node->get_visits()));
            row.emplace_back(std::string("勝率%"),
                node->get_visits() > 0 ?
                    std::to_string(node->get_mixed_score(color)*100.0f) :
                    std::string("-"));
            if (m_use_nets) {
                row.emplace_back(std::string("MC 勝率%"),
                    node->get_visits() > 0 ?
                    std::to_string(node->get_winrate(color)*100.0f) :
                    std::string("-"));
                row.emplace_back(std::string("Net 勝率%"),
                    node->get_evalcount() > 0 ?
                    std::to_string(node->get_eval(color)*100.0f) :
                    std::string("-"));
            }
            row.emplace_back(
                m_use_nets ? std::string("Net 確率%") : std::string("評価値"),
                std::to_string(node->get_score() * 100.0f));
            row.emplace_back(std::string("予想手順"), pvstring);
...
void UCTSearch::dump_analysis(void) {
    ...
    if (!m_quiet) {
        ...
        GUIprintf("ノード: %d, 勝率: %5.2f%%, 予想手順: %s", m_root.get_visits(),
                   mixrate, pvstring.c_str());
    } else {
        //GUIprintf("%d nodes searched", m_root.get_visits());
        GUIprintf("%d ノード探索", m_root.get_visits());
    }
}
...
int UCTSearch::think(int color, passflag_t passflag) {
    ...
    if (!m_analyzing) {
        ...
        GUIprintf("最大%.1f秒で思考中...", time_for_move/100.0f);
#ifdef KGS
    ...
    if (centiseconds_elapsed > 0) {
        ...
        GUIprintf("%d 訪問, %d ノード, %d プレイアウト, %d プレイアウト/秒",
                 m_root.get_visits(),
                  (int)m_nodes,
                  (int)m_playouts,
                 (m_playouts * 100) / (centiseconds_elapsed+1));
    ...
    GUIprintf("最善手: %s", m_rootstate.move_to_text(bestmove).c_str());
